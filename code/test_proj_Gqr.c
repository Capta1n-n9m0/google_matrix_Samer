...
struct node{
	double vector[NR];//hsr
	double vector2[NR];//hrs
	double vector3[NR];//Grs*P
...
typedef node* llist;
void add(llist *liste,int valeur,int value){
...
void creathss(llist *liste,int v[NR],int sum){
	llist tmp=*liste;
	int node1,node2;
	int j=0;
	int i=0;
...
int countnodes2(llist *liste,int sum){
...
void inithsr(llist *liste,int vecteur[NR]){
...
void inithrs(llist *liste,int vecteur[NR],int outlinks[NR]){
	int node1,node2,z,s;
	FILE*fz;
...
void copyvertical(llist *liste,int k,double *v){
	llist tmp=*liste;
	while(tmp!=NULL){
...
double modcopy(llist *liste,int k,double *v,double *a,double sigma){
	llist tmp=*liste;
	while(tmp!=NULL){
		v[tmp->index2-1]=tmp->vector2[k];
...
void recopyvertical(llist *liste,int k,double *v){
	llist tmp=*liste;
	while(tmp!=NULL){
...
void proj(llist *liste,double *a,double *b,double **t){
	int i,j;
	llist tmp=*liste;
	while(tmp!=NULL){
		for(i=0;i<NR;i++){
...
double calcul2(llist* liste,double *u,double *v,double x){
	llist tmp=*liste;
	while(tmp!=NULL){
		if(tmp->count==0){
...
double calcul3(llist* liste,double *u,double *v,double x){
	llist tmp=*liste;
	while(tmp!=NULL){
		if(tmp->count==0){
			x+=(double)alpha*u[tmp->index2-1];
...
void second_part(llist* liste,double** z,int k){
	int i,j;
	double* v;
	double* u;
	//double e=0;
	u=malloc(NR*sizeof(double));
	for(i=0;i<NR;i++){
		u[i]=0;
	}
...
void modsecond_part(llist* liste,double** z,int k,double** t){
	int i,j;
	double* v;
	double* u;
	u=malloc(NR*sizeof(double));
...
void dumpSomeNodes(llist *liste, int numberToDump){
	llist tmp=*liste;
	printf("dumping all node");
	int i = 0;
...
int main(){
	...
	//double H[NR][NR];
	double **H;
	H=malloc(NR*sizeof(double *));
	for(i=0;i<NR;i++){
		H[i]=malloc(NR*sizeof(double));
	}
	for(i=0;i<NR;i++){
		count[i]=0;
		for(j=0;j<NR;j++){
	...
	*/
	...
	double **second;
	second=malloc(NR*sizeof(double *));
	for(int i = 0 ; i < NR ; i++){
        second[i] = malloc(NR*sizeof(double));
	}
	for(i=0;i<NR;i++){
		count[i]=0;
		for(j=0;j<NR;j++){
	...
	*/
	...
	double* pi;
	double* prevpi;
	pi=malloc(NR*sizeof(double));
	prevpi=malloc(NR*sizeof(double));
	for(i=0;i<NR;i++){
		pi[i]=0;
		prevpi[i]=0;
	}
